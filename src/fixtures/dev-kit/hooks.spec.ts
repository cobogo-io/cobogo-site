...
import { useCurrency } from '../currency/functions/useCurrency'
import useSWR from 'swr'
import { toNaturalNumber, toBigNumber } from '../utility'
import {
...
jest.mock('swr')
jest.mock('../wallet/hooks.ts')
jest.mock('../dev-kit/client.ts')
jest.mock('../uniswap/client.ts')
jest.mock('../currency/functions/useCurrency')
...
describe('dev-kit hooks', () => {
  ...
  describe('useStakingShare', () => {
    ...
    test('success fetching data', () => {
      const data1 = '10000'
      const data2 = '5000'
      ;(useSWR ).mockImplementationOnce(() => ({ data: data1, error: undefined }))
      ;(useSWR ).mockImplementationOnce(() => ({ data: data2, error: undefined }))
      const { result } = renderHook(() => useStakingShare('property-address'))
    ...
    test('failure fetching market data', () => {
      ...
      const error = new Error(errorMessage)
      ;(useSWR ).mockImplementationOnce(() => ({ data: data1, error: undefined }))
      ;(useSWR ).mockImplementationOnce(() => ({ data: data2, error }))
      const { result } = renderHook(() => useStakingShare('property-address'))
      expect(result.current.error).toBe(error)
      expect(result.current.error?.message).toBe(errorMessage)
    })
  ...
  describe('useAPY', () => {
    ...
    test('success fetching data', () => {
      ...
      ;(useSWR ).mockImplementationOnce(() => ({ data: '10000', error: undefined }))
      ;(useSWR ).mockImplementationOnce(() => ({ data: '500000', error: undefined }))
      ;(useSWR ).mockImplementationOnce(() => ({ data: '9000', error: undefined }))
      const { result } = renderHook(() => useAPY())
  ...
  describe('useAnnualSupplyGrowthRatio', () => {
    ...
    test('success fetching data', () => {
      ...
      ;(useSWR ).mockImplementationOnce(() => ({ data: '10000', error: undefined }))
      ;(useSWR ).mockImplementationOnce(() => ({ data: '12000000', error: undefined }))
      const { result } = renderHook(() => useAnnualSupplyGrowthRatio())
  ...
  describe('useBalanceOf', () => {
    test('data is undefined and currency is DEV', () => {
      ...
      const toCurrency = (x?: BigNumber) => x
      ;(useCurrency as jest.Mock).mockImplementationOnce(() => ({ currency: 'DEV', toCurrency }))
      ;(useSWR ).mockImplementationOnce(() => ({ data, error }))
    ...
    test('success fetching data and currency is DEV', () => {
      ...
      ;(useSWR ).mockImplementationOnce(() => ({ data: toBigNumber('10000000000000000000') }))
      const { result } = renderHook(() => useBalanceOf())
